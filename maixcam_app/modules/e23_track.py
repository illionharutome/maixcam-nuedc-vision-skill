...
class E23TrackModule(VisionModule):
    ...
    def process(self, img) -> dict:
        ...
        target = self.target_detector.process(img)
        reference_mode = str(self.config.get("reference_mode", "image_center")).lower()
        reference = self.reference_detector.process(img) if self.reference_detector is not None else None
        if not target["ok"]:
    ...
    def draw_debug(self, img, result):
        target = result.get("extra", {}).get("target") or {}
        reference = result.get("extra", {}).get("reference") or {}
        if target.get("ok"):
